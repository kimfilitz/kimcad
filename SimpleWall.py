...
def get_modelview_matrix():
	...
	model_matrix = gpu.types.Buffer('FLOAT', [4, 4])
...
#
class SEBITEILE_OT_SIMPLEWALL(Operator):
	bl_idname = "mesh.sebiteile_simplewall"
	...
	dX: FloatProperty(name="dX", default = 0.2)
	dY: FloatProperty(name="dY", default = 0.2)
	
	angle: FloatProperty(name="angle", default = 45)
	length: FloatProperty(name="length", default = 0.2)
	
	valueX: FloatProperty(name="angle", default = 45)
	valueY: FloatProperty(name="length", default = 0.2)
	...
	def __del__(self):
		...
		print("  ::__del__ instance_type: "+str(self.instance_type))
		print("  ::__del__ dialog_state: "+str(self.get_dialog_state()))
		try:
	...
	def modal(self, context, event):
		print("SEBITEILE_OT_SIMPLEWALL::modal event.type: "+str(event.type) )
		print("SEBITEILE_OT_SIMPLEWALL::modal self.get_dialog_state(): "+str(self.get_dialog_state()) )
	...
	def invoke(self, context, event):
		print("SEBITEILE_OT_SIMPLEWALL::invoke event.type: "+str(event.type) )
		print("  ::invoke self.instance_type: "+str(self.instance_type) )
		...
		if (self.instance_type=="UNDEFINED"):
			bpy.ops.mesh.sebiteile_simplewall('INVOKE_DEFAULT',instance_type="DIALOG",)
			bpy.ops.mesh.sebiteile_simplewall('INVOKE_DEFAULT',instance_type="MODAL",)
			return {'FINISHED'}
...
def draw_callback(self, context):
		...
		print("  self.mouse_path: ",str(self.mouse_path))
...
def draw_callback_px(self, context):
	#print("mouse points", len(self.screen_path))
	print("::draw_callback_px self.mouse_path: ", len(self.screen_path))
	...
	if len(self.mouse_path) > 1:
		...
		currentPointPx = self.screen_path[-1]
		lastPointPx = self.screen_path[-2]
		
		d = math.sqrt( math.pow(currentPoint[0]-lastPoint[0], 2) + math.pow(currentPoint[1]-lastPoint[1], 2))
		
		dX = (currentPointPx[0] - lastPointPx[0])
		dY = (currentPointPx[1] - lastPointPx[1])
		...
		blf.position(font_id, lastPointPx[0] + dX/2, lastPointPx[1] + dY/2, 0)
...
def draw_callback_view(self, context):
		#print("mouse points", len(self.screen_path))
		print("::draw_callback_view self.mouse_path: ", len(self.mouse_path))
		...
		print("  system.dpi: ",str(system.dpi) , '  ', str(system.pixel_size))
		print("  dpi: ",str(dpi))
		...
		blf.enable(font_id, blf.ROTATION)
		blf.position(font_id, self.mouse_path[0].x+dX/2, self.mouse_path[0].y+dY/2, 0)
		blf.rotation(font_id, math.radians(angle))
		blf.aspect(font_id, 1.0)
		blf.size(font_id, 1, 500)
		blf.draw(font_id, str(d))
...
class SEBITEILE_OT_SIMPLEWALLADD(Operator):
	...
	def poll(cls, context):
		...
		print("  instance: "+str(SEBITEILE_OT_SIMPLEWALLADD.instance))
		...
		if SEBITEILE_OT_SIMPLEWALLADD.instance:
			print("  going to return False, already SEBITEILE_OT_SIMPLEWALLADD.instance ")
			return False
	...
	def execute(self, context):
		...
		self.report( {'INFO'}, 'F: %.2f  ' % (self.width) )
	...
	def modal(self, context, event):
		...
		print("  context.workspace.tools: "+str(context.workspace.tools))
		isactive = False
		for tool in context.workspace.tools:
			print("  tool: "+str(tool.idname))
			if tool.idname == 'sebiBauteile.parts_tool':
				isactive = True
		...
		if self.isValid(self.coord2, self.nonValidRegions ):
			...
			if event.type == 'MOUSEMOVE':
				...
				# at least one click
				if len(self.mouse_path) == 1: # add coord to have two points to draw
	...
	def update(self, context):
		...
		if consprops.constraint == 'UNDEFINED': # no constraints just set to mouse-location
			self.mouse_path[-1] = self.locPlane #
